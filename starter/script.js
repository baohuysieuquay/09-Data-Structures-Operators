...
console.log('Coding Challeng #1, 2, 3:');
...
document.querySelector('button').addEventListener('click', function () {
  ...
  let i = 0;
  for (const t of textSplitted) {
    i++;
    //const newText = t.trim().split('_').join('');
    const t2 = t.trim().toLowerCase();
    const firstPart = t2.slice(0, t2.indexOf('_'));
    const secondPart = t2.slice(t2.indexOf('_') + 1);
    const newText = firstPart + secondPart.replace(secondPart[0], secondPart[0].toUpperCase());
    console.log(newText.padEnd(22, ' ') + '✅'.repeat(i));
  }
